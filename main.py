...
# Khách bấm xem đơn (Bot tự động tính 70% quy đổi)
...
def show_orders(message):
    ...
    for idx, item in enumerate(orders, 1):
        hoan_tien = item['hoa_hong_goc'] * 0.7
        total += hoan_tien
        msg_text += f"{idx}. Đơn #{item['ma_don']}\n   💰 Tiền hoàn 70%: <b>+{hoan_tien:,.0f}đ</b>\n\n"
...
# Lệnh Admin cộng tiền siêu nhanh: /congtien <ID> <Mã_Đơn> <Hoa_Hồng_Gốc>
@bot.message_handler(commands=['congtien'])
def quick_add(message):
    if str(message.from_user.id) != str(ADMIN_ID):
        return
    try:
        _, user_id, ma_don, hoa_hong_goc = message.text.split()
        ...
        bot.reply_to(message, "❌ Dùng cú pháp: `/congtien [ID_Khách] [Mã_Đơn] [Hoa_Hồng_Adpia]`\nVí dụ: `/congtien 123456789 1500004652 23088`", parse_mode="Markdown")
